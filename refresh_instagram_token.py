...
（長期トークンは60日で期限切れ。30日以降に更新可能）

cron設定例（毎月1日 午前3時30分に実行）:
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(BASE_DIR, 'refresh_instagram_token.log')),
...
def update_env_file(env_path: str, new_token: str) -> bool:
    """.envファイルのINSTAGRAM_ACCESS_TOKENを書き換える"""
    try:
...
def main():
    ...
    if not current_token:
        logger.error('INSTAGRAM_ACCESS_TOKENが設定されていません')
        return

    logger.info('Instagramトークン更新を開始します')
    ...
    logger.info('Instagramトークン自動更新が完了しました')
